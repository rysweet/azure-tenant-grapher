...
def test_terraform_fixes():
    ...
    emitter_path = Path(__file__).parent / "src" / "iac" / "emitters" / "terraform_emitter.py"
    ...
    if '"resource_provider_registrations": "none"' in content:
        print("✅ Test 1 PASSED: Provider uses 'resource_provider_registrations': 'none'")
    else:
    ...
    if 'if not location or location.lower() == "none" or location.lower() == "null"' in content:
    ...
    for resource_type, prop in required_checks:
        if prop in content:
            print(f"✅ Test 5.{resource_type} PASSED: {resource_type} has required property {prop}")
        else:
            print(f"❌ Test 5.{resource_type} FAILED: {resource_type} missing required property")
            return False
