...
def test_create_tenant_command_structure():
    ...
    pattern = (
        r'@click\.command\("create-tenant"\).*?def\s+create_tenant_command\(.*?\):'
    )
    match = re.search(pattern, content, re.DOTALL)
...
def test_cli_help_output():
    ...
    assert "--num-entities" not in help_text, (
        "generate-sim-doc help should not show --num-entities"
    )
    assert "--num_entities" not in help_text, (
        "generate-sim-doc help should not show --num_entities"
    )
    ...
    if result.returncode == 0:
        help_text = result.stdout
        assert "MARKDOWN_FILE" in help_text.upper(), (
            "create-tenant help should show MARKDOWN_FILE argument"
        )
        assert "--num-entities" not in help_text, (
            "create-tenant help should not show --num-entities"
        )
...
def test_parameter_consistency_across_commands():
    ...
    for param in output_params:
        if param == "--out":
            pytest.fail(
                "Found '--out' parameter (should be '--output' for consistency). "
                "All output parameters should use '--output' not '--out'."
            )
