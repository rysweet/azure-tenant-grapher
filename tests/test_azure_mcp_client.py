...
def mock_tenant_manager():
    ...
    manager.get_current_tenant = AsyncMock(return_value=Tenant(
...
class TestFactoryFunctions:
    ...
    async def test_create_mcp_client(self, config):
        ...
        with patch('src.services.azure_mcp_client.AzureMCPClient.connect') as mock_connect:
            mock_connect.return_value = True
    ...
    async def test_integrated_natural_language_query(self, config, mcp_client):
        ...
        mcp_client.query_resources = AsyncMock(return_value={
            "status": "success",
            "results": [{"name": "test-vm"}]
        })
