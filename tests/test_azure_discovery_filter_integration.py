...
class TestAzureDiscoveryFilterIntegration:
    ...
    def mock_resource_groups(self):
        ...
        rgs = []
        names = ["rg-prod-001", "rg-dev-002", "rg-test-003", "rg-staging-004", "rg-demo-005"]
        for name in names:
            rgs.append({
                "name": name,
                "location": "eastus",
    ...
    @pytest.mark.asyncio
    async def test_discover_subscriptions_without_filter(self, discovery_service, mock_subscriptions):
        ...
        mock_sub_client.subscriptions.list.return_value = [
            Mock(subscription_id=s["subscription_id"], 
                 display_name=s["display_name"], 
                 state=s["state"])
            for s in mock_subscriptions
        ]
        discovery_service.subscription_client_factory = Mock(return_value=mock_sub_client)
    ...
    @pytest.mark.asyncio
    async def test_discover_subscriptions_with_filter(self, discovery_service, mock_subscriptions):
        """Test discovering subscriptions with filter returns only selected ones."""
        # Select specific subscriptions
        selected_ids = [mock_subscriptions[0]["subscription_id"], mock_subscriptions[2]["subscription_id"]]
        ...
        # Call with filter
        subscriptions = await discovery_service.discover_subscriptions(filter_config=filter_config)
        
        assert len(subscriptions) == 2
        sub_ids = [s["id"] for s in subscriptions]  # Note: returned dict has 'id' not 'subscription_id'
    ...
    async def test_discover_resources_in_subscription_without_rg_filter(
        ...
        mock_res_client.resources.list.return_value = [
            Mock(id=r["id"], name=r["name"], type=r["type"], 
        ...
        discovery_service._max_build_threads = 0  # Disable property enrichment for testing
    ...
    async def test_discover_resources_in_subscription_with_rg_filter(
        ...
        for r in mock_resources:
            ...
            if "/resourceGroups/" in resource["id"]:
                resource["resource_group"] = resource["id"].split("/resourceGroups/")[1].split("/")[0]
    ...
    async def test_full_discovery_workflow_with_filters(
        self, discovery_service, mock_subscriptions, mock_resources, mock_resource_groups
    ):
        ...
        filter_config = FilterConfig(
            ...
            resource_group_names=selected_rgs
        )
        ...
        for r in mock_resources:
            if "rg-prod-001" in r["id"]:
                ...
                matching = [fr for fr in filtered_mock_resources if fr["name"] == r["name"]]
                if matching:
    ...
    @pytest.mark.asyncio
    async def test_backward_compatibility_no_filter_service(self, mock_config, mock_credential, mock_subscriptions):
    ...
    async def test_empty_filter_config_returns_all(
        ...
        subscriptions = await discovery_service.discover_subscriptions(filter_config=empty_filter)
        assert len(subscriptions) == 5
    ...
    async def test_filter_config_from_environment_variables(self):
        ...
        filter_config = FilterConfig.from_comma_separated(
            subscription_ids=sub_ids_env,
            resource_group_names=rg_names_env
        )
