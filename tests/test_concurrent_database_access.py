...
class TestConcurrentDatabaseAccess:
    ...
    def test_concurrent_resource_state_operations(self):
        ...
        def worker_task(worker_id: int, results: List[bool]):
            ...
            try:
                for i in range(5):
                    ...
                    exists = resource_state.resource_exists(resource_id)
                    
                    # Test has_llm_description
                    has_desc = resource_state.has_llm_description(resource_id)
                    
                    # Test get_processing_metadata
                    metadata = resource_state.get_processing_metadata(resource_id)
        ...
        for thread_id, sessions in thread_sessions.items():
            # Check no session overlap between threads
            for session_id in sessions:
                assert session_id not in all_sessions_used, f"Session {session_id} was shared between threads"
                all_sessions_used.add(session_id)
        
        print(f"✅ Concurrent test passed: {num_workers} threads, {len(all_sessions_used)} unique sessions")
    ...
    def test_concurrent_database_operations(self):
        ...
        assert len(session_manager.active_sessions) == 0, "Some sessions were not properly closed"
    ...
    async def test_concurrent_resource_processor(self):
        ...
        mock_llm_generator.generate_resource_description.return_value = asyncio.coroutine(
            lambda: "Test LLM description"
        )()
        ...
        print(f"✅ Concurrent ResourceProcessor test passed: {len(thread_sessions)} threads used")
    ...
    def test_session_manager_stress_test(self):
        ...
        assert len(session_manager.active_sessions) == 0, "Sessions were not properly cleaned up"
        
        total_operations = sum(results)
        duration = end_time - start_time
        
        print(f"✅ Stress test passed: {num_workers} workers, {total_operations} operations in {duration:.2f}s")
...
def test_session_isolation_with_mock_neo4j_session_manager():
    ...
    class MockSession:
        ...
        def run(self, query, **params):
            ...
            time.sleep(0.001)
            mock_result = Mock()
            mock_result.single.return_value = {'count': 1, 'session_id': self.session_id}
            return mock_result
    ...
    print(f"✅ Session isolation test passed: {session_manager.sessions_created} sessions created and cleaned up")
