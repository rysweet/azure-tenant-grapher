...
def sample_tenant_spec():
    ...
    return {
        "tenant": {
            ...
            "groups": [{"id": "group-001", "display_name": "Admins"}],
            "service_principals": [{"id": "sp-001", "display_name": "App Service"}],
            "rbac_assignments": [
                {"principal_id": "user-001", "role": "Owner", "scope": "sub-001"}
            ],
            "relationships": [
                {"source_id": "user-001", "target_id": "group-001", "type": "MEMBER_OF"}
            ],
        }
    }
...
def test_create_tenant_displays_success_message(
    mock_tenant_creator, sample_tenant_spec
):
    ...
    try:
        with patch("src.cli_commands.ensure_neo4j_running"), patch(
            "src.llm_descriptions.create_llm_generator"
        ), patch("src.tenant_creator.TenantCreator") as MockCreator, patch(
            "src.tenant_creator.get_default_session_manager"
        ):
            # Setup mock to return our statistics
            MockCreator.return_value = mock_tenant_creator
            mock_tenant_creator.create_from_markdown = AsyncMock(
...
def test_create_tenant_displays_resource_counts(
    mock_tenant_creator, sample_tenant_spec
):
