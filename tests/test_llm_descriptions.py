...
from src.llm_descriptions import (
    LLMConfig, AzureLLMDescriptionGenerator, create_llm_generator
)
...
class TestAzureLLMDescriptionGenerator:
    ...
    async def test_generate_resource_description_success(self):
        ...
        mock_response.choices[0].message.content = "This is a mock Azure VM description."
    ...
    async def test_generate_relationship_description_success(self):
        ...
        mock_response = Mock()
        mock_response.choices = [Mock()]
        mock_response.choices[0].message.content = "VM depends on storage account for disk storage."
    ...
    async def test_process_resources_batch_success(self):
        ...
        with patch('src.llm_descriptions.AzureOpenAI', return_value=mock_client):
            ...
            enhanced_resources = await generator.process_resources_batch(resources, batch_size=2)
            
            assert len(enhanced_resources) == 2
            assert all('llm_description' in resource for resource in enhanced_resources)
            assert all(resource['llm_description'] == "Mock description" for resource in enhanced_resources)
    ...
    async def test_generate_tenant_specification(self):
        ...
        mock_response = Mock()
        mock_response.choices = [Mock()]
        mock_response.choices[0].message.content = "# Azure Tenant Specification\n\nThis is a mock specification."
...
class TestFactoryFunction:
    ...
    def test_create_llm_generator_exception(self):
        ...
        with patch.dict('os.environ', {
            ...
            with patch('src.llm_descriptions.AzureLLMDescriptionGenerator', side_effect=Exception("Init error")):
                generator = create_llm_generator()
                assert generator is None
