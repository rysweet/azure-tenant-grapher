...
from src.mcp_server import launch_mcp_server
...
class MCPServerTester:
    ...
    async def call_tool(self, tool_name: str, arguments: Dict[str, Any], request_id: int = 3) -> Dict[str, Any]:
    ...
    async def initialize_and_setup(self) -> None:
        ...
        if "error" in init_response:
            raise RuntimeError(f"Failed to initialize MCP session: {init_response['error']}")
...
async def test_mcp_multi_step_workflow(mcp_server_process: MCPServerTester):
    ...
    """
    storage_response = await tester.call_tool("read_neo4j_cypher", {"query": storage_query}, request_id=11)
    ...
    print(f"✅ Multi-step workflow succeeded: Found {result_data[0]['storage_count'] if result_data else 0} storage resources")
