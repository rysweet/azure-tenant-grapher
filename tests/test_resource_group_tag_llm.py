...
from src.llm_descriptions import AzureLLMDescriptionGenerator, LLMConfig
...
class TestResourceGroupTagLLM:
    ...
    @pytest.mark.asyncio
    async def test_generate_resource_group_summaries(self, mock_session_manager, mock_llm_generator):
        ...
        rg_result = Mock()
        rg_result.__iter__ = Mock(return_value=iter([
            {"name": "test-rg", "subscription_id": "sub-123"}
        ]))
        
        # Mock resources in ResourceGroup
        resources_result = Mock()
        resources_result.__iter__ = Mock(return_value=iter([
            {
                "name": "test-vm",
    ...
    @pytest.mark.asyncio
    async def test_generate_tag_summaries(self, mock_session_manager, mock_llm_generator):
        """Test Tag summary generation."""
        session_manager, session = mock_session_manager
        
        # Mock Tags needing descriptions
        tag_result = Mock()
        tag_result.__iter__ = Mock(return_value=iter([
            {"id": "Environment:Production", "key": "Environment", "value": "Production"}
        ]))
        
        # Mock tagged resources
        tagged_resources_result = Mock()
        tagged_resources_result.__iter__ = Mock(return_value=iter([
            {
                "name": "test-vm",
                "type": "Microsoft.Compute/virtualMachines",
                "location": "eastus",
    ...
    @pytest.mark.asyncio
    async def test_generate_summaries_with_empty_results(self, mock_session_manager, mock_llm_generator):
    ...
    @pytest.mark.asyncio
    async def test_generate_summaries_handles_exceptions(self, mock_session_manager, mock_llm_generator):
