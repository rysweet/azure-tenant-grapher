...
class TestAzureTenantGrapher:
    ...
    def test_initialization_with_llm(self, mock_config):
        ...
        with patch('src.azure_tenant_grapher.DefaultAzureCredential') as mock_cred:
            with patch('src.azure_tenant_grapher.Neo4jContainerManager') as mock_container:
                with patch('src.azure_tenant_grapher.create_llm_generator') as mock_llm_factory:
    ...
    def test_initialization_llm_failure(self, mock_config):
        ...
        with patch('src.azure_tenant_grapher.DefaultAzureCredential'):
            with patch('src.azure_tenant_grapher.Neo4jContainerManager'):
                with patch('src.azure_tenant_grapher.create_llm_generator', side_effect=Exception("LLM Error")):
                    grapher = AzureTenantGrapher(mock_config)
    ...
    def test_connect_to_neo4j_success(self, mock_config):
        ...
        with patch('src.azure_tenant_grapher.DefaultAzureCredential'):
            with patch('src.azure_tenant_grapher.Neo4jContainerManager') as mock_container_class:
    ...
    async def test_discover_subscriptions_success(self, mock_config):
        ...
        with patch('src.azure_tenant_grapher.DefaultAzureCredential'):
            with patch('src.azure_tenant_grapher.SubscriptionClient') as mock_sub_client:
                with patch('src.azure_tenant_grapher.create_llm_generator'):
    ...
    async def test_discover_resources_in_subscription_success(self, mock_config):
        ...
        with patch('src.azure_tenant_grapher.DefaultAzureCredential'):
            with patch('src.azure_tenant_grapher.ResourceManagementClient') as mock_resource_client:
                with patch('src.azure_tenant_grapher.create_llm_generator'):
                    # Setup mock resource
                    mock_resource = Mock()
                    ...
                    grapher = AzureTenantGrapher(mock_config)
                    resources = await grapher.discover_resources_in_subscription('test-sub-id')
    ...
    @pytest.mark.asyncio
    async def test_process_resources_with_enhanced_handling_success(self, mock_config, sample_resources):
            ...
            with patch('src.azure_tenant_grapher.create_llm_generator'):
                with patch.object(AzureTenantGrapher, 'connect_to_neo4j'):
                    with patch.object(AzureTenantGrapher, 'close_neo4j_connection'):
                        with patch('src.azure_tenant_grapher.create_resource_processor') as mock_processor_factory:
                            ...
                            mock_processor.process_resources_batch = AsyncMock(return_value=mock_stats)
                            mock_processor_factory.return_value = mock_processor
                            
                            grapher = AzureTenantGrapher(mock_config)
                            grapher.driver = Mock()
                            grapher.driver.session.return_value.__enter__ = Mock(return_value=Mock())
                            grapher.driver.session.return_value.__exit__ = Mock(return_value=None)
                            
                            result = await grapher.process_resources_with_enhanced_handling(sample_resources)
    ...
    async def test_build_graph_success(self, mock_config, sample_resources):
        ...
        with patch('src.azure_tenant_grapher.DefaultAzureCredential'):
            with patch('src.azure_tenant_grapher.create_llm_generator'):
                with patch.object(AzureTenantGrapher, 'connect_to_neo4j'):
                    with patch.object(AzureTenantGrapher, 'close_neo4j_connection'):
                        with patch.object(AzureTenantGrapher, 'discover_subscriptions') as mock_discover_subs:
                            with patch.object(AzureTenantGrapher, 'discover_resources_in_subscription') as mock_discover_resources:
                                with patch.object(AzureTenantGrapher, 'process_resources_with_enhanced_handling') as mock_process:
                                    with patch.object(AzureTenantGrapher, 'create_subscription_node'):
                                        # Setup mocks
                                        mock_discover_subs.return_value = [{'id': 'test-sub', 'display_name': 'Test Sub'}]
                                        mock_discover_resources.return_value = sample_resources
    ...
    async def test_build_graph_exception(self, mock_config):
        ...
        with patch('src.azure_tenant_grapher.DefaultAzureCredential'):
            with patch('src.azure_tenant_grapher.create_llm_generator'):
                with patch.object(AzureTenantGrapher, 'connect_to_neo4j', side_effect=Exception("Connection failed")):
                    with patch.object(AzureTenantGrapher, 'close_neo4j_connection'):
                        grapher = AzureTenantGrapher(mock_config)
                        result = await grapher.build_graph()
            ...
            with patch('src.azure_tenant_grapher.create_llm_generator') as mock_llm_factory:
                with patch.object(AzureTenantGrapher, 'connect_to_neo4j'):
                    with patch.object(AzureTenantGrapher, 'close_neo4j_connection'):
                        ...
                        mock_llm = Mock()
                        mock_llm.generate_tenant_specification = AsyncMock(return_value='/tmp/spec.md')
                        ...
                        mock_session.run.side_effect = [
                            Mock(__iter__=lambda x: iter([{'id': 'test', 'name': 'test'}])),  # resources
                            Mock(__iter__=lambda x: iter([{'relationship_type': 'CONTAINS'}]))  # relationships
                        ]
                        grapher.driver.session.return_value.__enter__ = Mock(return_value=mock_session)
                        grapher.driver.session.return_value.__exit__ = Mock(return_value=None)
