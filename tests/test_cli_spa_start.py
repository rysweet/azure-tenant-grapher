            ...
            with patch('src.cli_commands.os.path.dirname') as mock_dirname:
                ...
                with patch('src.cli_commands.subprocess.run') as mock_run:
                    ...
                    with patch('src.cli_commands.subprocess.Popen') as mock_popen:
                        ...
                        build_calls = [call for call in mock_run.call_args_list 
                                       if any('build' in str(arg) for arg in call[0][0])]
                        assert len(build_calls) > 0, f"Should run build command when dist doesn't exist. Calls: {mock_run.call_args_list}"
                ...
                with patch('src.cli_commands.subprocess.run') as mock_run:
                    ...
                    with patch('src.cli_commands.subprocess.Popen') as mock_popen:
                        ...
                        assert result.exit_code != 0 or "not found" in result.output.lower() or \
                               "doesn't exist" in result.output.lower(), \
            ...
            with patch('src.cli_commands.os.path.dirname') as mock_dirname:
                ...
                with patch('src.cli_commands.subprocess.run') as mock_run:
                    with patch('src.cli_commands.subprocess.Popen') as mock_popen:
                        ...
                        assert not any('build' in str(call) for call in mock_run.call_args_list), \
                    ...
                    with patch('src.cli_commands.subprocess.Popen') as mock_popen:
                        ...
                        def side_effect(*args, **kwargs):
                            if 'install' in str(args):
                                ...
                                (spa_dir / "dist" / "main").mkdir(parents=True, exist_ok=True)
                                (spa_dir / "dist" / "main" / "index.js").write_text("// Main")
                        ...
                        assert any('install' in str(call) for call in mock_run.call_args_list), \
