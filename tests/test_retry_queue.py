...
from src.resource_processor import ProcessingStats, ResourceProcessor
...
class DummySessionManager:
    def session(self):
        class DummySession:
            ...
            def run(self, *a, **k):
                return type("Result", (), {"single": lambda: {"count": 0}})()
...
async def test_retry_and_poison_queue(monkeypatch):
    ...
    poison_ids = []
    orig_logger_error = (
        processor.__class__.__dict__["process_resources"].__globals__["logger"].error
    )
    ...
    monkeypatch.setattr(
        processor.__class__.__dict__["process_resources"].__globals__["logger"],
        "error",
        fake_logger_error,
    )

    stats: ProcessingStats = await processor.process_resources(
        resources, max_workers=2, progress_callback=None, progress_every=1
    )
