...
class MCPServerTester:
    ...
    async def call_tool(self, tool_name: str, arguments: Dict[str, Any], request_id: int = 2) -> Dict[str, Any]:
...
@pytest.mark.asyncio
async def test_read_neo4j_cypher_tool_with_parameters(mcp_server_process: MCPServerTester):
    ...
    response = await tester.call_tool("read_neo4j_cypher", {"query": query, "params": params})
...
async def test_write_neo4j_cypher_tool(mcp_server_process: MCPServerTester):
    ...
    response = await tester.call_tool("write_neo4j_cypher", {"query": query, "params": params})
    ...
    await tester.call_tool("write_neo4j_cypher", {"query": cleanup_query, "params": params})
...
async def test_invalid_cypher_query_error_handling(mcp_server_process: MCPServerTester):
    ...
    assert "error" in response or ("result" in response and "isError" in response["result"])
...
async def test_multi_step_tool_usage(mcp_server_process: MCPServerTester):
    ...
    count_response = await tester.call_tool("read_neo4j_cypher", {"query": count_query}, request_id=11)
    ...
    """
    storage_response = await tester.call_tool("read_neo4j_cypher", {"query": storage_query}, request_id=12)
...
async def test_concurrent_tool_calls(mcp_server_process: MCPServerTester):
    ...
    for i in range(3):
        ...
        task = tester.call_tool("read_neo4j_cypher", {"query": query}, request_id=20 + i)
        tasks.append(task)
