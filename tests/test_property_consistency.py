...
def find_property_references_in_queries(session, label: str) -> Tuple[Set[str], List[str]]:
    """
    ...
    potential_properties = [
        ...
        "mail", "email", "jobTitle", "job_title",
        "description", "type", "objectId", "object_id"
    ]
    ...
    for prop in potential_properties:
        ...
        try:
            ...
            if "property" in str(e).lower():
                warnings.append(f"Property '{prop}' referenced but may not exist for {label}")
...
class TestPropertyConsistency:
    ...
    def test_user_properties_consistency(self, neo4j_session):
        ...
        assert not missing_properties, f"Missing expected properties for User: {missing_properties}"
        ...
        assert not incorrect_properties, f"Incorrect property names found: {incorrect_properties}"
    ...
    def test_query_property_references(self, neo4j_session):
        ...
        for query in invalid_queries:
            result = neo4j_session.run(query)
            records = list(result)
            assert len(records) == 0, f"Query with camelCase properties should not find results: {query}"
    ...
    def test_aad_graph_service_property_mapping(self, neo4j_session):
        ...
        neo4j_data = {
            ...
            "user_principal_name": azure_ad_data["userPrincipalName"],  # Converted to snake_case
        ...
        record = result.single()
        assert record["upn"] == "aad@example.com", "user_principal_name should be set correctly"
