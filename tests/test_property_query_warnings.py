        ...
        for prop_name, condition, should_work in property_tests:
            ...
            try:
                ...
                if should_work and not found:
                    results.append(
                        f"ERROR: Query with {prop_name} (snake_case) should work but found no results"
                    )
                elif not should_work and found:
                    results.append(
                        f"ERROR: Query with {prop_name} (camelCase) should not work but found results"
                    )
                else:
                    results.append(f"OK: Query with {prop_name} behaved as expected")

            except Exception as e:
                results.append(f"EXCEPTION: Query with {prop_name} raised: {e!s}")
