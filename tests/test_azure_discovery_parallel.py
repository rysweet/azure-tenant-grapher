...
class TestParallelPropertyFetching:
    ...
    def mock_resource_client(self):
        ...
        full_resource.properties.as_dict.return_value = {
            ...
            "osProfile": {"computerName": "testvm", "adminUsername": "azureuser"},
        }
    ...
    def discovery_service(self, mock_config):
        ...
        service = AzureDiscoveryService(
            config=mock_config, resource_client_factory=mock_resource_client_factory
        )
        return service
    ...
    @pytest.mark.asyncio
    async def test_get_api_version_for_resource(
        self, discovery_service, mock_resource_client
    ):
        ...
        # Should be cached
        assert (
            "Microsoft.Compute/virtualMachines" in discovery_service._api_version_cache
        )
        assert (
            discovery_service._api_version_cache["Microsoft.Compute/virtualMachines"]
            == "2023-03-01"
        )

    @pytest.mark.asyncio
    async def test_get_api_version_fallback(
        self, discovery_service, mock_resource_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_fetch_single_resource_with_properties_success(
        self, discovery_service, mock_resource_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_fetch_single_resource_handles_error(
        self, discovery_service, mock_resource_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_fetch_resources_with_properties_batch_processing(
        self, discovery_service, mock_resource_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_fetch_resources_respects_semaphore(
        self, discovery_service, mock_resource_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_discover_resources_without_parallel_fetching(
    ...
    @pytest.mark.asyncio
    async def test_batch_timeout_handling(
        self, discovery_service, mock_resource_client
    ):
        ...
        with patch.object(
            discovery_service, "_fetch_resources_with_properties"
        ) as mock_fetch:
