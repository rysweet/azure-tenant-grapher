...
class TestMCPServerManager:
    ...
    def manager(self):
        ...
        with patch.dict(
            os.environ, {"MCP_PORT": "8080", "MCP_ENDPOINT": "http://localhost:8080"}
        ):
    ...
    async def test_is_mcp_running_async_true(self, manager):
        ...
        with patch(
            "src.utils.mcp_startup.aiohttp.ClientSession", return_value=mock_session_cm
        ):
    ...
    async def test_start_mcp_server_async_success(self, manager):
        ...
        with patch.object(
            manager, "is_mcp_running_async", side_effect=[False, False, True]
        ):
