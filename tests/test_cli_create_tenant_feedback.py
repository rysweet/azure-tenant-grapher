...
@pytest.mark.skip(reason="LLM generates valid specs even from empty input, making it hard to test failure cases")
def test_create_tenant_shows_failure_feedback():
    ...
    try:
        ...
        assert "❌" in output or "failed" in output.lower() or "error" in output.lower(), \
