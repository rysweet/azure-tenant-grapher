...
@pytest.mark.skip(reason="DANGEROUS: Database tests disabled by default for safety. Set TEST_TENANT_SPEC_SAFE=true to enable with isolated containers.")
class TestTenantSpecificationGenerator:
    ...
    def isolated_neo4j_container(self) -> Generator[Tuple[str, str, str, str], None, None]:
        """
        ...
        if test_safe != "true":
            pytest.skip("TEST_TENANT_SPEC_SAFE must be set to 'true' to run these database tests")
        
        # Additional safety: Never run against production-like URIs
        test_uri = os.getenv("NEO4J_URI", "bolt://localhost:7688")
        if any(prod_indicator in test_uri.lower() for prod_indicator in ["prod", "production", "live"]):
            pytest.skip(f"Refusing to run tests against production-like URI: {test_uri}")
        ...
        if not container_manager.is_docker_available():
            pytest.skip("Docker is not available - cannot create isolated test container")
        
        if not container_manager.is_compose_available():
            pytest.skip("Docker Compose is not available - cannot create isolated test container")
    ...
    @pytest.fixture
    def setup_test_data(self, isolated_neo4j_container: Tuple[str, str, str, str]) -> Generator[Tuple[str, str, str, str], None, None]:
        """
    ...
    def test_spec_file_created_and_resource_limit(self, setup_test_data: Tuple[str, str, str, str]) -> None:
    ...
    def test_names_anonymized_and_no_real_ids(self, setup_test_data: Tuple[str, str, str, str]) -> None:
    ...
    def test_relationships_preserved(self, setup_test_data: Tuple[str, str, str, str]) -> None:
