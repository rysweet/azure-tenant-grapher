...
app.get('/api/dependencies', async (req, res) => {
  ...
  const dependencies = [
    { name: 'Python', command: 'python3 --version', required: '>=3.9' },
    { name: 'Docker', command: 'docker --version', required: 'any' },
    { name: 'Azure CLI', command: 'az --version', required: '>=2.0' },
    { name: 'Terraform', command: 'terraform --version', required: 'any' },
    { name: 'Bicep', command: 'bicep --version', required: 'any' }
  ];
  ...
  for (const dep of dependencies) {
    try {
      ...
      const version = extractVersionFromOutput(dep.name, stdout);
      results.push({
        name: dep.name,
        installed: true,
        version,
 ...
 */
function extractVersionFromOutput(toolName: string, output: string): string {
  try {
    ...
    switch (toolName) {
      case 'Python':
        ...
        return pythonMatch ? pythonMatch[1] : 'unknown';
        
      case 'Docker':
        // Docker version 24.0.0, build 1234
        const dockerMatch = output.match(/Docker version (\d+\.\d+\.\d+)/);
        return dockerMatch ? dockerMatch[1] : 'unknown';
        
      case 'Azure CLI':
        // Look for version in JSON output or first line
        if (output.includes('{')) {
          try {
            const jsonOutput = JSON.parse(output);
            return jsonOutput['azure-cli'] || 'unknown';
          } catch {
            // Fall back to regex
          }
        }
        const azMatch = output.match(/(\d+\.\d+\.\d+)/);
        return azMatch ? azMatch[1] : 'unknown';
        
      case 'Terraform':
        // Terraform v1.5.0
        const terraformMatch = output.match(/Terraform v(\d+\.\d+\.\d+)/);
        return terraformMatch ? terraformMatch[1] : 'unknown';
        
      case 'Bicep':
        // Bicep CLI version 0.20.4
        const bicepMatch = output.match(/Bicep CLI version (\d+\.\d+\.\d+)/);
        return bicepMatch ? bicepMatch[1] : 'unknown';
