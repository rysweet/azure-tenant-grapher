...
@click.option('--container-only', is_flag=True, help='Only start Neo4j container and exit')
@click.option('--visualize', is_flag=True, help='Generate graph visualization after building')
@click.option('--generate-spec', is_flag=True, help='Generate tenant specification after building')
@click.option('--log-level', default='INFO', help='Logging level (DEBUG, INFO, WARNING, ERROR)')
async def main(tenant_id, resource_limit, batch_size, no_container, 
               container_only, visualize, generate_spec, log_level):
    ...
    try:
        ...
        if result.get('success', False):
            ...
            if 'llm_descriptions_generated' in result:
                logger.info(f"   - LLM Descriptions: {result['llm_descriptions_generated']}")
            ...
            if generate_spec and config.azure_openai.is_configured():
                ...
                logger.warning("⚠️ Tenant specification requires Azure OpenAI configuration")
        else:
