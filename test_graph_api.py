...
async def test_graph_api():
    ...
    try:
        ...
        try:
            ...
            if users_result and users_result.value:
                ...
                if user_count > 0:
                    ...
                    logger.info(
                        f"   Sample user: {first_user.display_name} ({first_user.user_principal_name})"
                    )
            else:
                logger.warning("⚠️  No users returned (might be empty tenant)")
        except ODataError as e:
            logger.error(
                f"❌ Cannot read users: {e.error.message if e.error else str(e)}"
            )
            logger.error(f"   Status code: {e.response_status_code}")
            if e.response_status_code == 403:
                logger.error(
                    "   Permission needed: User.Read.All or Directory.Read.All"
                )
        ...
        try:
            ...
            logger.error(
                f"❌ Cannot read groups: {e.error.message if e.error else str(e)}"
            )
            logger.error(f"   Status code: {e.response_status_code}")
            if e.response_status_code == 403:
                logger.error(
                    "   Permission needed: Group.Read.All or Directory.Read.All"
                )
        ...
        try:
            ...
            if sp_result and sp_result.value:
                ...
                logger.info(
                    f"✅ Can read service principals (found {sp_count} on first page)"
                )
                if sp_count > 0:
            ...
            logger.error(
                f"❌ Cannot read service principals: {e.error.message if e.error else str(e)}"
            )
            logger.error(f"   Status code: {e.response_status_code}")
            if e.response_status_code == 403:
                logger.error(
                    "   Permission needed: Application.Read.All or Directory.Read.All"
                )
        ...
        try:
            ...
            logger.error(
                f"❌ Cannot read directory roles: {e.error.message if e.error else str(e)}"
            )
            logger.error(f"   Status code: {e.response_status_code}")
            if e.response_status_code == 403:
                logger.error(
                    "   Permission needed: RoleManagement.Read.Directory or Directory.Read.All"
                )
        ...
        logger.info(
            "    - Directory.Read.All (covers users, groups, service principals, roles)"
        )
