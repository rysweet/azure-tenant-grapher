...
class Neo4jConfig:
    ...
    uri: str = field(default_factory=lambda: os.getenv('NEO4J_URI', 'bolt://localhost:7688'))
    user: str = field(default_factory=lambda: os.getenv('NEO4J_USER', 'neo4j'))
    password: str = field(default_factory=lambda: os.getenv('NEO4J_PASSWORD', 'azure-grapher-2024'))
...
class AzureOpenAIConfig:
    ...
    api_version: str = field(default_factory=lambda: os.getenv('AZURE_OPENAI_API_VERSION', '2025-04-16'))
    model_chat: str = field(default_factory=lambda: os.getenv('AZURE_OPENAI_MODEL_CHAT', 'gpt-4'))
    model_reasoning: str = field(default_factory=lambda: os.getenv('AZURE_OPENAI_MODEL_REASONING', 'gpt-4'))
    ...
    def validate(self) -> None:
        ...
        if not self.is_configured():
            raise ValueError("Azure OpenAI configuration is incomplete. Please set AZURE_OPENAI_ENDPOINT and AZURE_OPENAI_KEY")
...
class ProcessingConfig:
    ...
    batch_size: int = field(default_factory=lambda: int(os.getenv('PROCESSING_BATCH_SIZE', '5')))
    max_retries: int = field(default_factory=lambda: int(os.getenv('PROCESSING_MAX_RETRIES', '3')))
    retry_delay: float = field(default_factory=lambda: float(os.getenv('PROCESSING_RETRY_DELAY', '1.0')))
    parallel_processing: bool = field(default_factory=lambda: os.getenv('PROCESSING_PARALLEL', 'true').lower() == 'true')
    auto_start_container: bool = field(default_factory=lambda: os.getenv('AUTO_START_CONTAINER', 'true').lower() == 'true')
...
class LoggingConfig:
    ...
    format: str = field(default_factory=lambda: os.getenv('LOG_FORMAT', '%(log_color)s%(levelname)s:%(name)s:%(message)s'))
    file_output: Optional[str] = field(default_factory=lambda: os.getenv('LOG_FILE', None))
...
class AzureTenantGrapherConfig:
    ...
    @classmethod
    def from_environment(cls, tenant_id: str, resource_limit: Optional[int] = None) -> 'AzureTenantGrapherConfig':
        """
    ...
    def log_configuration_summary(self) -> None:
        ...
        logger.info(f"   - Resource Limit: {self.processing.resource_limit or 'Unlimited'}")
        ...
        logger.info(f"   - Auto Start Container: {self.processing.auto_start_container}")
...
def setup_logging(config: LoggingConfig) -> None:
    ...
    logger.info(f"📝 Logging configured: level={config.level}, file={config.file_output or 'console'}")


def create_config_from_env(tenant_id: str, resource_limit: Optional[int] = None) -> AzureTenantGrapherConfig:
    """
