...
def get_source_credentials() -> dict[str, str]:
    """
    Get source tenant credentials for discovery operations.

    This function provides backward compatibility:
    - If AZURE_SOURCE_TENANT_* variables are set, uses those
    - Otherwise falls back to AZURE_TENANT_ID, AZURE_CLIENT_ID, AZURE_CLIENT_SECRET

    Returns:
        Dictionary with source tenant credentials:
        - tenant_id: Source tenant ID
        - client_id: Source service principal client ID
        - client_secret: Source service principal client secret
        - subscription_id: Source subscription ID (optional)

    Raises:
        ValueError: If credentials are not configured
    """
    # Try dual-tenant source credentials first
    tenant_id = os.getenv("AZURE_SOURCE_TENANT_ID")
    client_id = os.getenv("AZURE_SOURCE_TENANT_CLIENT_ID")
    client_secret = os.getenv("AZURE_SOURCE_TENANT_CLIENT_SECRET")
    subscription_id = os.getenv("AZURE_SOURCE_TENANT_SUBSCRIPTION_ID")

    # Fallback to single-tenant credentials
    if not tenant_id:
    ...
    if not tenant_id:
        raise ValueError(
            "Source tenant ID not configured. Set AZURE_SOURCE_TENANT_ID or AZURE_TENANT_ID"
        )
    if not client_id:
        raise ValueError(
            "Source client ID not configured. Set AZURE_SOURCE_TENANT_CLIENT_ID or AZURE_CLIENT_ID"
        )
    if not client_secret:
        raise ValueError(
            "Source client secret not configured. Set AZURE_SOURCE_TENANT_CLIENT_SECRET or AZURE_CLIENT_SECRET"
        )
...
def get_target_credentials() -> dict[str, str]:
    """
    Get target tenant credentials for deployment operations.

    This function provides backward compatibility:
    - If AZURE_TARGET_TENANT_* variables are set, uses those
    - Otherwise falls back to AZURE_TENANT_ID, AZURE_CLIENT_ID, AZURE_CLIENT_SECRET

    Returns:
        Dictionary with target tenant credentials:
        - tenant_id: Target tenant ID
        - client_id: Target service principal client ID
        - client_secret: Target service principal client secret
        - subscription_id: Target subscription ID (optional)

    Raises:
        ValueError: If credentials are not configured
    """
    # Try dual-tenant target credentials first
    tenant_id = os.getenv("AZURE_TARGET_TENANT_ID")
    client_id = os.getenv("AZURE_TARGET_TENANT_CLIENT_ID")
    client_secret = os.getenv("AZURE_TARGET_TENANT_CLIENT_SECRET")
    subscription_id = os.getenv("AZURE_TARGET_TENANT_SUBSCRIPTION_ID")
    ...
    if not tenant_id:
        raise ValueError(
            "Target tenant ID not configured. Set AZURE_TARGET_TENANT_ID or AZURE_TENANT_ID"
        )
    if not client_id:
        raise ValueError(
            "Target client ID not configured. Set AZURE_TARGET_TENANT_CLIENT_ID or AZURE_CLIENT_ID"
        )
    if not client_secret:
        raise ValueError(
            "Target client secret not configured. Set AZURE_TARGET_TENANT_CLIENT_SECRET or AZURE_CLIENT_SECRET"
        )
