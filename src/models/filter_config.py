...
class FilterConfig(BaseModel):
    ...
    def validate_subscription_ids(cls, v: Optional[List[str]]) -> List[str]:
        ...
        for sub_id in v:
            try:
                ...
                raise ValueError(f"Invalid subscription ID format: {sub_id}. Must be a valid UUID.")
    ...
    def validate_resource_group_names(cls, v: Optional[List[str]]) -> List[str]:
        ...
        pattern = re.compile(r"^[a-zA-Z0-9][a-zA-Z0-9._()-]{0,88}[a-zA-Z0-9_)]$|^[a-zA-Z0-9]$")
    ...
    def should_include_resource(self, resource: dict) -> bool:
        ...
        if len(parts) > 4 and parts[3] == 'resourceGroups':
            ...
            if self.resource_group_names and resource_group not in self.resource_group_names:
                return False
