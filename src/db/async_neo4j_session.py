...
class AsyncNeo4jSession:
    ...
    async def __aenter__(self) -> Any:  # noqa: D401
        ...
        self._session = await session_obj if asyncio.iscoroutine(session_obj) else session_obj
    ...
    async def run(self, *args: Any, **kwargs: Any) -> Any:  # noqa: D401
        ...
        if not self._session:
            raise RuntimeError("Session has not been initialised; use within an async context manager.")
