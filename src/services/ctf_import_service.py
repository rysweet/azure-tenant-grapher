...
class CTFImportService:
    ...
    def import_terraform(
        ...
        if terraform_dir:
            ...
            warnings.append(
                "Terraform directory parsing not yet supported. "
                "Use terraform.tfstate file instead."
            )
        ...
        for resource in resources:
            ...
            if not resource_id:
                warnings.append(
                    f"Resource missing ID: {resource.get('name', 'unknown')}"
                )
                continue
            ...
            if not resource_role:
                resource_role = self.annotation_service.determine_role(
                    resource.get("resource_type", ""), resource.get("name", "")
                )
        ...
        return {
            "success": resources_imported > 0
            or (resources_imported == 0 and resources_failed == 0),
    ...
    def _annotate_resources_batch(
        ...
        for resource in resources:
            try:
                ...
                failure_count += 1
                failed_resources.append({"id": resource["id"], "error": str(e)})
