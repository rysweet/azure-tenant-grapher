...
class AzureThreatAnalysisRunner:
    ...
    def analyze_from_graph(self, logger: Optional[logging.Logger] = None) -> List[Dict[str, Any]]:
        """
        ...
        if not self.session_manager:
            logger.warning("No Neo4j session manager provided, cannot analyze from graph")
            return []
    ...
    def _parse_dfd_components(self, dfd_spec: str, logger: logging.Logger) -> List[Dict[str, Any]]:
        """
        Parse DFD components from specification.
    ...
    def _infer_component_type(self, label: str) -> str:
        ...
        if any(keyword in label_lower for keyword in ['web app', 'webapp', 'api', 'app service']):
        ...
        elif any(keyword in label_lower for keyword in ['vm', 'virtual machine', 'server']):
            return "Microsoft.Compute/virtualMachines"
        elif any(keyword in label_lower for keyword in ['network', 'vnet', 'subnet']):
            return "Microsoft.Network/virtualNetworks"
        elif any(keyword in label_lower for keyword in ['key vault', 'keyvault', 'secrets']):
            return "Microsoft.KeyVault/vaults"
        else:
            return "Microsoft.Resources/resourceGroups"  # Default fallback

    def _convert_dfd_component_to_resource(self, component: Dict[str, Any]) -> Dict[str, Any]:
        """
...
def run_tmt(
    ...
    try:
        ...
        if dfd_artifact and (dfd_artifact.endswith('.tm7') or dfd_artifact.endswith('.json') or dfd_artifact.endswith('.md')):
            try:
                with open(dfd_artifact, 'r', encoding='utf-8') as f:
                    dfd_content = f.read()
                return runner.analyze_from_dfd_specification(dfd_content, logger)
            except FileNotFoundError:
                logger.warning(f"DFD file not found: {dfd_artifact}, treating as specification string")
