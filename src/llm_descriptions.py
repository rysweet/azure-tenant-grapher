...
class AzureLLMDescriptionGenerator:
    ...
    def __init__(self, config: Optional[LLMConfig] = None):
        ...
        if not self.config.is_valid():
            raise ValueError("Invalid LLM configuration. Please check your environment variables.")
        ...
        logger.info(f"Initialized Azure LLM Description Generator with endpoint: {self.base_url}")
    ...
    async def generate_resource_description(self, resource_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            resource_summary = {
                ...
                'type': resource_type,
                'location': location,
                'key_properties': {k: v for k, v in properties.items() if k in [
                    'sku', 'size', 'tier', 'state', 'status', 'provisioningState',
                    'publicNetworkAccess', 'minimumTlsVersion', 'allowBlobPublicAccess'
                ]},
                'tags': tags
            }
...
"""
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are an expert Azure cloud architect who creates clear, concise descriptions of Azure resources for technical documentation."},
            ...
            logger.debug(f"Generated description for {resource_type} '{resource_name}': {description}")
            
            return description
            
        except Exception as e:
            logger.error(f"Failed to generate description for resource {resource_data.get('name', 'Unknown')}: {str(e)}")
            return f"Azure {resource_type} resource providing cloud services and functionality."
    
    async def generate_relationship_description(self, 
...
"""
            ...
            response = self.client.chat.completions.create(
                model=self.config.model_chat,
                messages=[
                    {"role": "system", "content": "You are an expert Azure cloud architect explaining resource relationships."},
                    {"role": "user", "content": prompt}
                ],
            ...
            description = response.choices[0].message.content.strip()
            logger.debug(f"Generated relationship description: {relationship_type} between {source_type} and {target_type}")
            
            return description
            
        except Exception as e:
            logger.error(f"Failed to generate relationship description: {str(e)}")
...
"""
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are a senior Azure cloud architect creating detailed infrastructure documentation."},
                    {"role": "user", "content": prompt}
                ],
            ...
            logger.error(f"Failed to generate tenant specification: {str(e)}")
            # Create a basic fallback specification
            fallback_spec = self._create_fallback_specification(resources, relationships)
            with open(output_path, 'w', encoding='utf-8') as f:
...
"""
    ...
    async def process_resources_batch(self, 
                                    resources: List[Dict[str, Any]], 
        ...
        """
        total_resources = len(resources)
        logger.info(f"🔄 Starting LLM description generation for {total_resources} resources (batch size: {batch_size})")
        
        processed_count = 0
        successful_count = 0
        failed_count = 0
        
        async def process_resource(resource, resource_index):
            ...
            try:
                logger.info(f"📝 Generating description for resource {resource_index + 1}/{total_resources}: {resource_name} ({resource_type})")
                ...
                logger.info(f"✅ Successfully described {resource_name}: \"{desc_preview}\"")
                ...
                logger.error(f"❌ Failed to generate description for {resource_name} ({resource_type}): {str(e)}")
        ...
        for batch_start in range(0, total_resources, batch_size):
            ...
            batch_with_indices = [(resource, batch_start + idx) for idx, resource in enumerate(batch)]
            batch_results = await asyncio.gather(*[
                process_resource(resource, resource_index) 
                for resource, resource_index in batch_with_indices
            ])
            processed_resources.extend(batch_results)
            
            # Progress summary for this batch
            logger.info(f"📊 Batch {batch_number} complete: {len(batch_results)} resources processed. Overall progress: {processed_count}/{total_resources} ({(processed_count/total_resources)*100:.1f}%)")
            
            # Small delay between batches to respect rate limits
            if batch_end < total_resources:
                logger.info(f"⏳ Waiting 2 seconds before next batch to respect API rate limits...")
                await asyncio.sleep(2)
        
        # Final summary
        logger.info(f"🎉 LLM description generation completed!")
        logger.info(f"📈 Summary: {successful_count} successful, {failed_count} failed, {processed_count} total")
        logger.info(f"✨ Success rate: {(successful_count/total_resources)*100:.1f}%")
...
def create_llm_generator() -> Optional[AzureLLMDescriptionGenerator]:
    ...
    try:
        ...
        if not config.is_valid():
            logger.warning("LLM configuration is invalid. Skipping LLM description generation.")
            return None
        
        return AzureLLMDescriptionGenerator(config)
    except Exception as e:
        logger.error(f"Failed to create LLM generator: {str(e)}")
        return None
