...
def create_tenant_from_markdown(text: str):
    ...
    async def _run():
        ...
        try:
            ...
            stats = await creator.ingest_to_graph(spec, is_llm_generated=is_llm_generated)
            return stats
...
def create_tenant_command(markdown_file: str):
    ...
    try:
        ...
        click.echo("\n✅ Tenant successfully created in Neo4j!")
        
        if stats:
            click.echo("\n📊 Resources Created:")
            click.echo(f"  • Tenant: {stats.get('tenant', 0)}")
            click.echo(f"  • Subscriptions: {stats.get('subscriptions', 0)}")
            click.echo(f"  • Resource Groups: {stats.get('resource_groups', 0)}")
            click.echo(f"  • Resources: {stats.get('resources', 0)}")
            click.echo(f"  • Users: {stats.get('users', 0)}")
            click.echo(f"  • Groups: {stats.get('groups', 0)}")
            click.echo(f"  • Service Principals: {stats.get('service_principals', 0)}")
            click.echo(f"  • Managed Identities: {stats.get('managed_identities', 0)}")
            click.echo(f"  • Admin Units: {stats.get('admin_units', 0)}")
            click.echo(f"  • RBAC Assignments: {stats.get('rbac_assignments', 0)}")
            click.echo(f"  • Relationships: {stats.get('relationships', 0)}")
            click.echo(f"\n  Total: {stats.get('total', 0)} entities created")
        else:
            click.echo("\nTenant graph created successfully.")
    except Exception as e:
        click.echo(
