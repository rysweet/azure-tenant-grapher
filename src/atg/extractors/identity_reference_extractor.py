...
class IdentityReferenceExtractor:
    ...
    def extract_from_resources(self, resources: List[Dict[str, Any]]) -> Dict[str, Set[str]]:
        """
        ...
        logger.info(f"Found {len(self.user_assigned_identity_resource_ids)} user-assigned identity resource IDs")
        ...
        if user_assigned_identities and isinstance(user_assigned_identities, dict):
            for resource_id, identity_info in user_assigned_identities.items():
                ...
                if isinstance(identity_info, dict):
                    ...
                    if user_principal_id and user_principal_id != '00000000-0000-0000-0000-000000000000':
                        self.identity_references.add(user_principal_id)
                        logger.debug(f"Found user-assigned identity: {user_principal_id} from {resource_id}")
    ...
    def _extract_by_resource_type(self, resource: Dict[str, Any], resource_type: str) -> None:
        """
    ...
    def _extract_from_logic_app(self, resource: Dict[str, Any]) -> None:
        ...
        if isinstance(parameters, dict):
            for param_name, param_value in parameters.items():
                if isinstance(param_value, dict) and param_value.get('type') == 'ManagedServiceIdentity':
