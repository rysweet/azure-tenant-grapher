...
class MCPServerManager:
    ...
    def __init__(self, debug: bool = False):
        ...
        self.mcp_endpoint = os.environ.get("MCP_ENDPOINT", f"http://localhost:{self.mcp_port}")
    ...
    async def is_mcp_running_async(self) -> bool:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.mcp_endpoint}/health", timeout=2) as response:
    ...
    def start_mcp_server(self) -> bool:
        ...
        try:
            ...
            for attempt in range(max_attempts):
                time.sleep(1)
                if self.is_mcp_running():
                    logger.info(f"✅ MCP server started successfully on port {self.mcp_port}")
                    return True
                
                # Check if process has died
                if self.process.poll() is not None:
                    stdout, stderr = self.process.communicate()
                    error_msg = f"MCP server process died. Exit code: {self.process.returncode}"
                    if stderr:
                        error_msg += f"\nError: {stderr.decode()}"
                ...
                if self.debug:
                    logger.debug(f"Waiting for MCP server... (attempt {attempt + 1}/{max_attempts})")
