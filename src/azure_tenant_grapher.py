...
class AzureTenantGrapher:
    ...
    def __init__(self, config: AzureTenantGrapherConfig):
        ...
        if config.azure_openai.is_configured():
            ...
            self.llm_generator = None
            logger.info("ℹ️ Azure OpenAI not configured - LLM descriptions disabled")
    ...
    async def generate_tenant_specification(self):
        ...
        if not self.llm_generator:
            logger.warning("⚠️ LLM generator not available, skipping tenant specification generation")
            return
    ...
    async def process_resources_with_enhanced_handling(self, resources: List[Dict]) -> Dict:
        """
        ...
        if not resources:
            ...
            return {'total_resources': 0, 'successful': 0, 'failed': 0, 'success_rate': 100.0}
        
        logger.info(f"🔄 Starting enhanced resource processing for {len(resources)} resources")
        ...
        try:
            ...
            with self.driver.session() as session:
                ...
                for subscription in subscriptions:
                    logger.info(f"📋 Creating subscription node: {subscription['display_name']}")
                ...
                for subscription in subscriptions:
                    ...
                    try:
                        ...
                        logger.info(f"✅ Added {len(resources)} resources from {subscription_name}")
                        
                    except Exception as e:
                        logger.error(f"❌ Error processing subscription {subscription_name}: {e}")
                        continue
            
            # Process all resources with enhanced handling
            if all_resources:
                logger.info(f"🔄 Starting processing of {len(all_resources)} total resources")
                processing_stats = await self.process_resources_with_enhanced_handling(all_resources)
                ...
                return {
                    ...
                    'llm_descriptions_generated': processing_stats.get('llm_generated', 0),
