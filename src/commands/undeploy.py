...
@click.option(
    '--deployment-id',
    help='ID of deployment to destroy'
)
@click.option(
    '--directory',
    help='IaC output directory to destroy'
)
@click.option(
    '--tenant',
...
@click.option(
    '--no-backup',
    is_flag=True,
    help='Skip state backup before destruction'
)
def undeploy(
    ...
    if not deployment_id and not directory:
        click.echo("❌ Error: Must specify either --deployment-id or --directory", err=True)
        sys.exit(1)
    
    if deployment_id and directory:
        click.echo("❌ Error: Cannot specify both --deployment-id and --directory", err=True)
        sys.exit(1)
    ...
    if deployment_id:
        ...
        if not deployment:
            ...
            click.echo("⚠️  Warning: This directory is not tracked in the deployment registry")
            deployment = {
    ...
    if exit_code == 0:
        ...
        if deployment['id'].startswith('deploy-'):
            registry.mark_destroyed(
                deployment['id'],
                backup_path=str(backup_path) if not no_backup and 'backup_path' in locals() else None
            )
