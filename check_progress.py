...
def check_database_progress():
    ...
    try:
        ...
        with driver.session() as session:
            ...
            if resource_count > 0:
                progress_pct = (llm_count / resource_count) * 100 if resource_count > 0 else 0
                print(f"📈 LLM Progress: {progress_pct:.1f}% ({llm_count}/{resource_count})")
                ...
                for record in result:
                    name = record['r.name'][:30] + "..." if len(record['r.name']) > 30 else record['r.name']
                    resource_type = record['r.type'].split('/')[-1] if '/' in record['r.type'] else record['r.type']
                    location = record['r.location']
                    has_llm = record['has_llm']
                    print(f"   • {name} ({resource_type}) in {location} - LLM: {has_llm}")
                ...
                if llm_count > 0:
                    ...
                    for record in result:
                        name = record['r.name'][:25] + "..." if len(record['r.name']) > 25 else record['r.name']
                        resource_type = record['r.type'].split('/')[-1] if '/' in record['r.type'] else record['r.type']
                        desc_preview = record['r.llm_description'][:100] + "..." if len(record['r.llm_description']) > 100 else record['r.llm_description']
