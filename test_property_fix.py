...
def main():
    ...
    issues = check_file_properties(
        "src/tenant_creator.py",
        "User",
        user_properties
    )
    ...
    if issues:
        ...
        with open("src/tenant_creator.py", 'r') as f:
            content = f.read()
        ...
        if found_properties:
            print(f"  ✅ Correct! Using camelCase properties: {', '.join(found_properties)}")
        else:
            print("  ⚠️ No User properties found")
    
    # Check aad_graph_service.py
    print("\n2. Checking aad_graph_service.py:")
    with open("src/services/aad_graph_service.py", 'r') as f:
        content = f.read()
    ...
    if found_aad_props:
        print(f"  ✅ Correct! Using camelCase in props dictionary: {', '.join(found_aad_props)}")
    else:
    ...
    print("- User nodes will have: displayName, userPrincipalName, jobTitle, mailNickname")
