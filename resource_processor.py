...
class ResourceState:
    ...
    def has_llm_description(self, resource_id: str) -> bool:
        ...
        try:
            ...
            if record:
                desc = record['desc']
                return desc is not None and desc.strip() != '' and not desc.startswith('Azure ')
            return False
        except Exception as e:
...
class DatabaseOperations:
    ...
    def upsert_resource(self, resource: Dict, processing_status: str = "completed") -> bool:
        """
        ...
        try:
            ...
            logger.error(f"Error upserting resource {resource.get('id', 'Unknown')}: {e}")
            return False
    
    def create_subscription_relationship(self, subscription_id: str, resource_id: str) -> bool:
        ...
        try:
            ...
            MERGE (s)-[:CONTAINS]->(r)
            """
            self.session.run(query, subscription_id=subscription_id, resource_id=resource_id)
            return True
        except Exception as e:
            logger.error(f"Error creating subscription relationship for {resource_id}: {e}")
            return False
    ...
    def create_resource_group_relationships(self, resource: Dict) -> bool:
        ...
        try:
            ...
            MERGE (s)-[:CONTAINS]->(rg)
            """
            self.session.run(sub_rg_query, subscription_id=subscription_id, rg_name=rg_name)
            ...
            MERGE (rg)-[:CONTAINS]->(r)
            """
            self.session.run(rg_resource_query, rg_name=rg_name, subscription_id=subscription_id, resource_id=resource_id)
            
            return True
            
        except Exception as e:
            logger.error(f"Error creating resource group relationships for {resource.get('id', 'Unknown')}: {e}")
            return False
...
class ResourceProcessor:
    ...
    def __init__(self, session: Any, llm_generator: Optional[AzureLLMDescriptionGenerator] = None, resource_limit: Optional[int] = None):
        """
        ...
        logger.info(f"Initialized ResourceProcessor with LLM: {'enabled' if llm_generator else 'disabled'}")
    ...
    async def _process_single_resource_llm(self, resource: Dict) -> Tuple[bool, str]:
        ...
        try:
            description = await self.llm_generator.generate_resource_description(resource)
            return True, description
        except Exception as e:
            logger.error(f"LLM generation failed for {resource.get('name', 'Unknown')}: {str(e)}")
            return False, f"Azure {resource.get('type', 'Resource')} resource."
    
    async def process_single_resource(self, resource: Dict, resource_index: int) -> bool:
        """
        ...
        try:
            ...
            if not should_process:
                logger.info(f"⏭️  Resource {resource_index + 1}/{self.stats.total_resources}: {resource_name} - SKIPPED ({reason})")
                self.stats.skipped += 1
                return True
            
            logger.info(f"📝 Processing resource {resource_index + 1}/{self.stats.total_resources}: {resource_name} ({resource_type}) - {reason}")
            ...
            if reason in ["new_resource", "needs_llm_description", "retry_failed"]:
                ...
                llm_success, description = await self._process_single_resource_llm(resource)
            ...
            success = self.db_ops.upsert_resource(resource, processing_status="completed")
            if not success:
                raise Exception("Failed to upsert resource")
            
            # Create relationships
            self.db_ops.create_subscription_relationship(resource['subscription_id'], resource_id)
            ...
            logger.error(f"❌ Failed to process resource {resource_name} ({resource_type}): {str(e)}")
    ...
    async def process_resources_batch(self, resources: List[Dict], batch_size: int = 5) -> ProcessingStats:
        """
        ...
        if self.resource_limit and len(resources) > self.resource_limit:
            logger.info(f"🔢 Limiting processing to {self.resource_limit} resources (found {len(resources)})")
            resources = resources[:self.resource_limit]
        
        self.stats.total_resources = len(resources)
        
        if not resources:
            logger.info("ℹ️  No resources to process")
            return self.stats
        
        logger.info(f"🔄 Starting parallel batch processing of {self.stats.total_resources} resources (batch size: {batch_size})")
        ...
        for batch_start in range(0, self.stats.total_resources, batch_size):
            ...
            logger.info(f"🔄 Processing batch {batch_number}/{total_batches} (resources {batch_start + 1}-{batch_end})")
            ...
            for idx, result in enumerate(batch_results):
                if isinstance(result, Exception):
                    resource = batch[idx]
                    logger.error(f"❌ Batch processing exception for {resource.get('name', 'Unknown')}: {result}")
                    self.stats.failed += 1
                    self.stats.processed += 1
    ...
    def _log_progress_summary(self, batch_number: int, total_batches: int):
        """Log progress summary for the current batch."""
        logger.info(f"📊 Batch {batch_number}/{total_batches} complete. Progress: {self.stats.processed}/{self.stats.total_resources} ({self.stats.progress_percentage:.1f}%)")
        logger.info(f"   ✅ Success: {self.stats.successful} | ❌ Failed: {self.stats.failed} | ⏭️  Skipped: {self.stats.skipped}")
        if self.llm_generator:
            logger.info(f"   🤖 LLM Generated: {self.stats.llm_generated} | ⚠️  LLM Skipped: {self.stats.llm_skipped}")
    ...
    def _log_final_summary(self):
        ...
        logger.info(f"✅ Successful: {self.stats.successful} ({self.stats.success_rate:.1f}%)")
...
def create_resource_processor(session: Any, llm_generator: Optional[AzureLLMDescriptionGenerator] = None, resource_limit: Optional[int] = None) -> ResourceProcessor:
    """
