...
async def demonstrate_mcp_capabilities():
    ...
    mcp_client = await create_mcp_client(
        config=config, tenant_manager=tenant_manager, auto_connect=True
    )
    ...
    try:
        ...
        for tenant in tenants:
            logger.info(
                f"  Found tenant: {tenant.get('display_name')} ({tenant.get('tenant_id')})"
            )
        ...
        if identity_info["status"] == "success":
            ...
            logger.info(
                f"  Permissions: {', '.join(identity_info.get('permissions', []))}"
            )
...
async def main():
    ...
    parser.add_argument(
        "--interactive", action="store_true", help="Run in interactive query mode"
    )
