...
class OrchestrationCycle:
    ...
    def send_imessage(self, message: str) -> None:
        ...
        try:
            subprocess.run(
                [str(IMESS_TOOL), message], timeout=10, check=False, capture_output=True
            )
        except Exception:
            pass
...
"""
    ...
    def check_and_cleanup_agents(self) -> None:
        ...
        for i, agent in enumerate(self.active_agents):
            ...
            if poll is not None:
                ...
                self.log(
                    "INFO",
                    f"✅ Agent {agent.name} completed (duration: {duration:.0f}s)",
                )
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    f"ITERATION{next_num}_",
                    "--skip-name-validation",
                    "--output",
                    f"demos/iteration{next_num}",
                ],
                capture_output=True,
                text=True,
                timeout=300,
    ...
    def terraform_apply_async(self, iteration: int) -> Optional[int]:
        ...
        try:
            ...
            self.log(
                "INFO", f"Started apply iteration {iteration} (PID: {process.pid})"
            )
    ...
    def evaluate_objective(self) -> bool:
        ...
        self.log(
            "INFO",
            f"📊 Source: {source_count} | Target: {target_count} | Fidelity: {fidelity:.1f}%",
        )
        ...
        self.log(
            "INFO",
            f"🎯 Objective: {'✅ ACHIEVED' if criteria_met else '🔄 In Progress'}",
        )
        self.log(
            "INFO",
            f"   - Fidelity >=95%: {'✅' if fidelity_met else '❌'} ({fidelity:.1f}%)",
        )
        self.log(
            "INFO",
            f"   - 3 passes: {'✅' if consecutive_complete >= 3 else '❌'} ({consecutive_complete}/3)",
        )
    ...
    def progress_iteration(self, iteration: int, status: str) -> None:
        ...
        if status == "complete":
            ...
            self.log(
                "INFO", f"Iteration {iteration} complete - scanning target tenant..."
            )
    ...
    def run_cycle(self) -> bool:
        ...
        for pid_file in ORCHESTRATOR_STATE.glob("terraform_apply_*.pid"):
            try:
                ...
                if match:
                    ...
                    self.log(
                        "INFO",
                        f"Terraform apply for iteration {iter_num} is still running (PID: {pid})",
                    )
                break
            except (OSError, ValueError):
                pid_file.unlink(missing_ok=True)
        ...
        if self.active_agents:
            self.log(
                "INFO",
                f"Active agents: {', '.join(a.name for a in self.active_agents)}",
            )
...
def main():
    ...
    orchestrator.log(
        "INFO", "🚀 Orchestrator Started - Running Until 100% Objective Achieved"
    )
    orchestrator.send_imessage(
        "🚀 Orchestrator started - running continuously until 100% objective achieved"
    )
