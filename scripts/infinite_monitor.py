...
def main():
    ...
    send_message(
        "🔄 Infinite monitoring loop started - will continue until objective achieved"
    )
    ...
    while True:
        ...
        print(
            f"[{timestamp}] Cycle {cycle}: {status}, {created} created, {errors} errors"
        )

        # Send updates
        if status == "complete":
            send_message(
                f"🎉 DEPLOYMENT COMPLETE! {created} resources created. Proceeding to verification..."
            )
        ...
        elif created > last_created + 50 or (
            now - last_update > 300 and created > last_created
        ):
