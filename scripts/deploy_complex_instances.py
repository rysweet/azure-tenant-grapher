#!/usr/bin/env python3
"""
Deploy High-Complexity Architecture Instances to Target Tenant
...
3. Filters to instances with deployment complexity score >= 4.0 (the hard ones)
4. Deletes all existing resource groups in the target subscription
5. Deploys the selected high-complexity instances via Terraform
...
Usage:
    ...
    python scripts/deploy_complex_instances.py

    # Subsequent runs (loads cached plan, skips analysis):
    python scripts/deploy_complex_instances.py

    # Force plan regeneration:
    python scripts/deploy_complex_instances.py --refresh-plan

    # Dry run (no deletion/deployment):
    python scripts/deploy_complex_instances.py --dry-run
...
PLAN_CACHE_FILE = PROJECT_ROOT / "output" / "complex_deployment_plan_cache.json"
...
def compute_incremental_spectral_distances(
    high_complexity_instances: list[tuple[str, list[dict]]],
    replicator: ArchitecturePatternReplicator,
    analyzer: ArchitecturalPatternAnalyzer,
) -> tuple[list[int], list[float]]:
    """Compute spectral distance as we incrementally add each high-complexity instance."""
    ...
    logger.info("Computing incremental spectral distance across %d instances...", len(high_complexity_instances))

    for pattern_name, instance in high_complexity_instances:
...
def print_deployment_table(
    ...
    for i, ((pattern, instance), score) in enumerate(zip(instances, scores), 1):
        ...
        flag = " *" if combined < min_score else ""
        print(f"{i:>3}  {pattern:<30}  {combined:>6.2f}  {n_res:>10}  {band}{flag}")
    print("=" * len(header))
    high = sum(1 for s in scores if s.get("combined_score", 0) >= min_score)
    print(f"Total instances: {len(instances)}  ({high} high-complexity ≥{min_score}, {len(instances)-high} fill-in *)")
...
def main() -> int:
    ...
    parser.add_argument("--min-score", type=float, default=DEFAULT_MIN_SCORE,
                        help=f"Preferred minimum deployment score (default: {DEFAULT_MIN_SCORE})")
    parser.add_argument("--target-resource-count", type=int, default=500,
                        help="Target total resources to deploy (default: 500). "
                             "High-complexity instances (>= min-score) are prioritised; "
                             "lower-complexity instances fill the gap if needed.")
    ...
    output_dir = PROJECT_ROOT / "output" / f"complex_deployment_{timestamp}"
    ...
    logger.info("=" * 70)
    logger.info(
        "STEP 3: Scoring instances — targeting %d resources (min score %.1f preferred)...",
    ...
    # Sort descending by score so high-complexity instances come first
    all_scored.sort(key=lambda x: x[2]["combined_score"], reverse=True)

    # Greedily select instances until we hit the target resource count.
    # Instances with score >= min_score are always included first; lower-scored
    # ones pad the remainder up to target_resource_count.
    high_complexity: list[tuple[str, list[dict]]] = []
    ...
    for pattern_name, instance, score in all_scored:
        if total_resources >= args.target_resource_count:
            break
        # Always include if score >= min_score; include lower-scored to fill gap
        high_complexity.append((pattern_name, instance))
        high_scores.append(score)
        total_resources += score.get("resource_count", len(instance))

    high_score_count = sum(1 for s in high_scores if s["combined_score"] >= args.min_score)
    logger.info(
        "Selected %d instances (%d with score >= %.1f, %d fill-in) = %d resources.",
        len(high_complexity),
        high_score_count,
        args.min_score,
        len(high_complexity) - high_score_count,
        total_resources,
    )
    ...
    before = len(high_complexity)
    deployable_instances = filter_deployable_instances(high_complexity)
    ...
    paired_filtered = [
        ...
        for (pname, inst_list), score in zip(high_complexity, high_scores)
        if id(inst_list) in deployable_inst_ids
    ]
    high_complexity = [p[0] for p in paired_filtered]
    high_scores = [p[1] for p in paired_filtered]
    logger.info(
    ...
    print_deployment_table(high_complexity, high_scores, min_score=args.min_score)

    # Save filtered plan alongside timestamped output
    plan_path = output_dir / "filtered_high_complexity_plan.json"
    with open(plan_path, "w") as f:
        json.dump(
            [
                ...
                for (p, inst), s in zip(high_complexity, high_scores)
            ],
            f,
            indent=2,
        )
    ...
    if not args.skip_spectral:
        ...
        cumulative_resources, spectral_distances = compute_incremental_spectral_distances(
            high_complexity, replicator, analyzer
        )
    ...
    # ── Step 6: Deploy ─────────────────────────────────────────────────────────
    logger.info("=" * 70)
    logger.info("STEP 6: Deploying %d high-complexity instances...", len(high_complexity))
    logger.info("=" * 70)
    ...
    if args.dry_run:
        ...
        logger.info(
            ...
            len(high_complexity),
            sum(s.get("resource_count", 0) for s in high_scores),
            TARGET_SUBSCRIPTION,
        )
        return 0

    # Build TenantGraph from Neo4j (full resource properties including location)
    tenant_graph = fetch_full_resources_from_neo4j(high_complexity)
    ...
    try:
        deployment_result = deploy_terraform(
            ...
            resource_group="complex-architecture-replication",
            location="eastus",
            dry_run=False,
            subscription_id=TARGET_SUBSCRIPTION,
            verbose=True,
    ...
    summary = {
        ...
        "instances_deployed": len(high_complexity),
        "total_resources": sum(s.get("resource_count", 0) for s in high_scores),
        "mean_score": sum(s.get("combined_score", 0) for s in high_scores) / max(len(high_scores), 1),
    ...
    print(f"  Instances deployed:  {len(high_complexity)}")
    print(f"  Resources deployed:  {sum(s.get('resource_count', 0) for s in high_scores)}")
