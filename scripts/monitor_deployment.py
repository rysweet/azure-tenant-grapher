...
def main():
    ...
    while True:
        ...
        if status == "complete":
            send_message(
                f"🎉 Deployment COMPLETE! {created} resources created, {errors} errors"
            )
            break

        # Send update every 5 minutes or when significant progress is made
        now = time.time()
        if created > last_created + 50 or (
            now - last_update_time > 300 and created > last_created
        ):
            send_message(
                f"⏳ Progress: {created}/619 resources created ({created * 100 // 619}%), {errors} errors"
            )
