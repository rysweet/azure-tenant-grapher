...
def run_command(
    cmd: List[str], cwd: Path = REPO_ROOT, timeout: int = 300, check: bool = False
) -> Tuple[int, str, str]:
...
def get_latest_iteration_num() -> int:
    ...
    iterations = [
        ...
        and d.name.replace("iteration", "").isdigit()
    ]
    if not iterations:
        return 0
...
def generate_iteration(iteration_num: int) -> bool:
    ...
    if code == 0:
        ...
        if main_tf.exists() and main_tf.stat().st_size > 1000:
            log("⚠️ Generation command failed but files exist, continuing")
            return True
...
def validate_iteration(iteration_num: int) -> Tuple[bool, List[str]]:
    ...
    code, stdout, stderr = run_command(
        ["terraform", "init", "-upgrade"], cwd=iteration_dir, timeout=180
    )
    if code != 0:
        ...
        try:
            if status["phase"] == "iteration":
                ...
                if is_valid:
                    ...
                    log(
                        f"✅ Validation passed! Consecutive passes: {status['consecutive_passes']}"
                    )
                else:
                    status["consecutive_passes"] = 0
                    log(f"❌ Validation failed with {len(errors)} errors")
                    send_message(
                        f"⚠️ Iteration {next_iter} failed validation ({len(errors)} errors)"
                    )
                ...
                if status["consecutive_passes"] >= 3:
                    ...
                    log(
                        f"Need {3 - status['consecutive_passes']} more consecutive passes"
                    )
