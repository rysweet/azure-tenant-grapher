...
def fix_dual_graph_index():
    ...
    for line in lines:
        if (
            "DUAL_GRAPH_DESIGN_SUMMARY.md" in line
            or "DUAL_GRAPH_IMPLEMENTATION_STRATEGY.md" in line
        ):
            continue
...
def fix_monitor_command():
    ...
    if "VISUALIZATION.md" in content or "BUILD_COMMAND.md" in content:
        content = (
            content.replace("[", "")
            .replace("](VISUALIZATION.md)", "")
            .replace("](BUILD_COMMAND.md)", "")
        )
...
def fix_synthetic_node():
    ...
    content = content.replace(
        "../spa/backend/README.md", "../spa/backend/src/ (backend source code)"
    )
    file_path.write_text(content)
...
def fix_architecture_dual_graph():
    ...
    content = file_path.read_text()
    content = content.replace(
        "(scan-source-node-relationships.md)", "(scan-source-node-relationships.md)"
    )
    file_path.write_text(content)
...
def fix_scan_source_node():
    ...
    content = file_path.read_text()
    content = content.replace(
        "../../src/iac/README.md", "../../src/iac/ (IaC generation source)"
    )
    file_path.write_text(content)
...
def fix_demo_overview():
    ...
    content = file_path.read_text()
    content = content.replace(
        "commands/appendix.md", "commands/ (see individual command files)"
    )
    file_path.write_text(content)
...
def fix_config_quick_start():
    ...
    if not design_file.exists():
        content = content.replace(
            "(CONFIG_SYSTEM_DESIGN.md)", "(see config system documentation)"
        )
    file_path.write_text(content)
...
def fix_diagram_manifest():
    ...
    if not png_file.exists():
        content = content.replace(
            "(dual-graph-architecture.png)", "(see DUAL_GRAPH_ARCHITECTURE_DIAGRAM.txt)"
        )
    file_path.write_text(content)
...
def fix_agent_vs_manual():
    ...
    content = file_path.read_text()
    content = content.replace(
        "(AUTONOMOUS_DEPLOYMENT_FAQ.md)", "(AUTONOMOUS_DEPLOYMENT_FAQ.md)"
    )
    file_path.write_text(content)
...
def fix_autonomous_deployment_faq():
    ...
    content = content.replace(
        "(AUTONOMOUS_DEPLOYMENT.md)", "(AUTONOMOUS_DEPLOYMENT.md)"
    )
    file_path.write_text(content)
...
def fix_autonomous_deployment_quick_ref():
    ...
    content = file_path.read_text()
    content = content.replace(
        "(AGENT_DEPLOYMENT_TUTORIAL.md)", "(AGENT_DEPLOYMENT_TUTORIAL.md)"
    )
    file_path.write_text(content)
