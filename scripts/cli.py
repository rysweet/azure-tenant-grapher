...
@click.option(
    ...
    is_flag=True,
    help="Automatically import pre-existing resources in target subscription to avoid conflicts",
)
@click.option(
    "--import-strategy",
    type=click.Choice(["resource_groups", "all_resources", "selective"], case_sensitive=False),
    default="resource_groups",
    help="Import strategy for existing resources (default: resource_groups)",
)
@click.pass_context
@async_command
@click.option(
