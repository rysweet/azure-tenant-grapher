...
"""
    ...
    prompt_file = PROJECT_ROOT / f".claude/prompts/gap_{gap_type}_{int(time.time())}.txt"
...
def main():
    ...
    while True:
        ...
        print(f"Objective Status: {'✅ ACHIEVED' if objective_eval['achieved'] else '🔄 In Progress'}")
        print(f"Fidelity: {objective_eval['fidelity']:.1f}% ({objective_eval['target_resources']}/{objective_eval['source_resources']})")
        ...
        if objective_eval['achieved']:
            send_imessage(f"🎉 OBJECTIVE ACHIEVED! Fidelity: {objective_eval['fidelity']:.1f}%")
        ...
        if latest_iter > 0:
            ...
            if iter_status.get('status') == 'applied':
                ...
                cmd = [
                    ...
                    "--resource-group-prefix", f"ITERATION{latest_iter + 1}_",
                    "--skip-name-validation",
                    "--output", f"demos/iteration{latest_iter + 1}"
                ]
                
                try:
                    result = subprocess.run(cmd, check=True, capture_output=True, text=True, timeout=300, cwd=PROJECT_ROOT)
