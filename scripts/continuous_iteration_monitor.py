...
class IterationMonitor:
    ...
    def generate_iteration(self) -> Tuple[bool, Path, str]:
        ...
        cmd = [
            ...
            f"ITERATION{self.iteration_num}_",
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.error(
                    f"❌ Iteration {self.iteration_num} generation failed: {error}"
                )
    ...
    def validate_iteration(self, iteration_dir: Path) -> Tuple[bool, int, List[str]]:
        ...
        try:
            ...
            logger.warning(
                f"❌ {iteration_dir.name} has {error_count} validation errors"
            )
    ...
    def run_iteration_cycle(self):
        ...
        if not success:
            self._send_imessage(
                f"❌ ITER {self.iteration_num} generation failed: {message}"
            )
        ...
        logger.info(
            f"Error categories: {json.dumps({k: len(v) for k, v in error_categories.items()}, indent=2)}"
        )
    ...
    def run_continuous_loop(self, max_iterations: int = 1000):
        ...
        """
        logger.info(
            f"Starting continuous iteration loop (max {max_iterations} iterations)"
        )
        self._send_imessage(
            f"🚀 Starting continuous iteration from ITER {self.iteration_num}"
        )

        consecutive_passes = 0
        required_consecutive_passes = (
            3  # Need 3 consecutive passes to confirm stability
        )

        while self.iteration_num <= max_iterations:
            logger.info(f"\n{'=' * 60}")
            logger.info(
                f"ITERATION {self.iteration_num} - {datetime.now().strftime('%H:%M:%S')}"
            )
            logger.info(f"{'=' * 60}")
...
def main():
    ...
    try:
        ...
        logger.info("\n🛑 Stopped by user")
        monitor._send_imessage(
            f"🛑 Continuous iteration stopped at ITER {monitor.iteration_num}"
        )
    except Exception as e:
        logger.error(f"Fatal error: {e}", exc_info=True)
        monitor._send_imessage(
            f"💥 Fatal error at ITER {monitor.iteration_num}: {str(e)[:100]}"
        )
        sys.exit(1)
