...
dashboard = RichDashboard(
    config=config,
    max_llm_threads=10,
    max_build_threads=25
)
assert dashboard.max_llm_threads == 10, f"Expected max_llm_threads=10, got {dashboard.max_llm_threads}"
assert dashboard.max_build_threads == 25, f"Expected max_build_threads=25, got {dashboard.max_build_threads}"
assert dashboard.max_concurrency == 10, f"Expected max_concurrency=10 (backward compat), got {dashboard.max_concurrency}"
...
dashboard_old = RichDashboard(
    config=config,
    max_concurrency=15
)
assert dashboard_old.max_llm_threads == 15, f"Expected max_llm_threads=15, got {dashboard_old.max_llm_threads}"
assert dashboard_old.max_build_threads == 20, f"Expected max_build_threads=20 (default), got {dashboard_old.max_build_threads}"
assert dashboard_old.max_concurrency == 15, f"Expected max_concurrency=15, got {dashboard_old.max_concurrency}"
...
try:
    ...
    assert "Max Concurrency" not in output, "Old Max Concurrency still shown in config panel"
