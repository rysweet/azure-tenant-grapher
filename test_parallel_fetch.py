...
async def test_parallel_fetching():
    ...
    config = AzureTenantGrapherConfig(
        ...
        processing=ProcessingConfig(max_build_threads=5)
    )
    
    print(f"✅ Config created with max_build_threads={config.processing.max_build_threads}")
    
    # Create discovery service
    service = AzureDiscoveryService(config)
    print(f"✅ Discovery service created with _max_build_threads={service._max_build_threads}")
    
    # Verify parallel fetching is enabled
    assert service._max_build_threads == 5, f"Expected 5, got {service._max_build_threads}"
    print("✅ Parallel fetching is enabled!")
    
    # Test that _fetch_resources_with_properties exists
    assert hasattr(service, '_fetch_resources_with_properties'), "Missing _fetch_resources_with_properties"
    print("✅ _fetch_resources_with_properties method exists")
    
    # Test that _get_api_version_for_resource exists
    assert hasattr(service, '_get_api_version_for_resource'), "Missing _get_api_version_for_resource"
