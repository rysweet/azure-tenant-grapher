...
def send_message(msg):
    ...
    try:
        subprocess.run([os.path.expanduser("~/.local/bin/imessR"), msg], 
...
def validate_iteration(iteration_num):
    ...
    try:
        ...
        for err in errors[:10]:  # First 10 errors
            error_summary.append(f"{err.get('severity')}: {err.get('summary')} - {err.get('detail', '')[:100]}")
...
def main():
    ...
    while iteration <= max_iterations:
        ...
        if success:
            ...
            send_message(f"🎉 VALIDATION SUCCESS! ITERATION {iteration} is ready to deploy!")
            ...
            send_message(f"⚠️ ITERATION {iteration} failed. Moving to {iteration + 1}. Errors: {len(errors)}")
